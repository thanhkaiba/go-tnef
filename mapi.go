...
func decodeMapi(data []byte, startingOffset int) (attrs []MAPIAttribute, err error) {
	defer func() {
	...
	numProperties := binary.LittleEndian.Uint32(data[offset:])
	...
	for i := 0; i < int(numProperties); i++ {
		...
		attrType := binary.LittleEndian.Uint16(data[offset:])
		offset += 2

		attrName := binary.LittleEndian.Uint16(data[offset:])
		offset += 2

		guid := 0
		if attrName >= GUID_EXISTS_FLAG && attrName <= 0xFFFE {
			...
			if kind == 0 {
				...
				iidLen := binary.LittleEndian.Uint32(data[offset:])
				offset += 4
				r := iidLen % 4
				if r != 0 {
					iidLen += 4 - r
		...
		isMulti := false
		numMVProperties := 1
		if MULTI_VALUE_FLAG&attrType != 0 {
			attrType ^= MULTI_VALUE_FLAG
			numMVProperties = int(binary.LittleEndian.Uint32(data[offset:]))
			isMulti = true
			offset += 4
		}

		if numMVProperties > 1024 && numMVProperties > len(data) {
			return nil, fmt.Errorf("count is too large: %d", numMVProperties)
		}

		var attrData []byte
		for i := 0; i < numMVProperties; i++ {
			attrData, offset = parseProperty(data, offset, attrName, attrType, isMulti)
			attrs = append(attrs, MAPIAttribute{Type: attrType, Name: attrName, Data: attrData, GUID: guid})
		}

		if (numMVProperties%2 == 0 || !isMulti) && (attrType == szmapiShort || attrType == szmapiBoolean) {
			offset += 2
		}
	}
...
func parseProperty(data []byte, offset int, attr_name, attr_type uint16, is_multi bool) ([]byte, int) {
	attr_data := []byte{}
	...
	case szmapiString,
		...
		szmapiUnspecified:
		{
			num_vals := 1
			if !is_multi {
				num_vals = int(binary.LittleEndian.Uint32(data[offset:]))
				offset += 4
			}
...
const (
	MULTI_VALUE_FLAG = 0x1000 // OR with type means multiple values
...
var (
	IMessageSig    = []byte{0x07, 0x03, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x46}
	IMessageSigLen = len(IMessageSig)
)
